...
if ($respuestaUsuarios['ok'] && is_array($respuestaUsuarios['data'])) {
    foreach ($respuestaUsuarios['data'] as $u) {
        if (!empty($u['username'])) {
            $usuariosPorUsername[$u['username']] = $u['fullName'] ?? '';
        }
    }
}
...
<?php else: ?>
    ...
    <form class="filter-card" method="GET">
        ...
        <div class="filter-actions">
            ...
            <a href="/modules/rrhh/guardias/registros/" class="btn-secondary">
                Limpiar
            </a>
        </div>
                ...
                <tbody>
                    <?php foreach ($registrosPagina as $r): ?>
                        ...
                        <tr>
                            ...
                            <?php
                            $usernameRegistro = $r['username'] ?? '';
                            $nombreUsuario = $usuariosPorUsername[$usernameRegistro] ?? '-';
                            ?>

                            <td><?= htmlspecialchars($usernameRegistro ?: '-') ?></td>
                    ...
                    <?php if (count($registrosPagina) === 0): ?>
                        <tr>
                            <td colspan="9">No hay registros para los filtros seleccionados.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
