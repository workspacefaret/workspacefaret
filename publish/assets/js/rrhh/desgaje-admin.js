document.addEventListener('DOMContentLoaded', () => {
        ...
        if (esAdminTi) {
            tablaBody.querySelectorAll('button[data-eliminar-id]').forEach(btn => {
                btn.addEventListener('click', (evento) => {
                    ...
                    eliminarRegistro(btn.dataset.eliminarId, btn.dataset.eliminarCodigo);
                });
            });
        }
    }

    async function eliminarRegistro(id, referencia) {
        const confirmado = confirm(
            `¿Eliminar definitivamente el registro de desgaje ${referencia || id}?\n\nSe borrará el registro completo (todos sus trabajos/NPs), su firma, PDF y adjuntos. Esta acción NO se puede deshacer.`
        );
    ...
    function descripcionTrabajo(detalle) {
        if (detalle.tipoDesgajeId) {
