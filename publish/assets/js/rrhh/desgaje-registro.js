    ...
    form.addEventListener('submit', async (event) => {
        ...
        for (let i = 0; i < filas.length; i++) {
            ...
            if (fila.dataset.modo === 'MANUAL') {
                ...
                const cantidadBuena = Number(fila.querySelector('[data-field="cantidadBuena"]').value || 0);
                const cantidadMala = Number(fila.querySelector('[data-field="cantidadMala"]').value || 0);
                ...
                if (cantidadBuena < 0 || cantidadMala < 0) {
                    mostrarAlerta('error', `La cantidad procesada buena y mala no pueden ser negativas (trabajo ${numeroFila}).`);
                    return;
                }

                if (cantidadBuena + cantidadMala <= 0 || precioTrabajo <= 0) {
                    mostrarAlerta('error', `La cantidad procesada (buena + mala) y el precio del trabajo deben ser mayores a 0 (trabajo ${numeroFila}).`);
                    return;
                }
